...
def user_del(id=None):
    ...
    db.session.commit()
    if os.path.exists(current_app.config['UP_DIR'] + "users" + os.sep + str(user.face)):
        os.remove(current_app.config['UP_DIR'] + "users" + os.sep + str(user.face))
    flash("删除会员成功！", "ok")
    oplog = Oplog(
...
def comment_list(page=None):
    ...
    #
    page_data = Comment.query.order_by(Comment.addtime.desc()).paginate(page=page, per_page=current_app.config['PAGE_SET'])
...
def post_list(page=None):
    ...
    page_data = Post.query.order_by(
        Post.addtime.desc()
    ).paginate(page=page, per_page=current_app.config['PAGE_SET'])
